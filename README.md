...
- **解析器链架构** —— 四阶段流水线：Hosts → 过滤器 → 缓存 → 上游转发，职责清晰、易于扩展
- **LRU + TTL 双过期缓存** —— 最近最少使用淘汰 + 生存时间过期，最大化缓存命中率
- **Trie 树域名过滤** —— 高效的黑白名单匹配，支持 `*.example.com` 通配符
- **上游 DNS 轮询与故障转移** —— Round-Robin 负载分散 + 超时重试 + 自动故障转移
- **跨平台 I/O** —— Windows 使用 IOCP 完成端口，Linux 使用 epoll 边缘触发
- **Linux Daemon 模式** —— 启动后自动脱离终端，作为守护进程运行
- **运行时监控** —— 统计 QPS、缓存命中率、拦截数、转发成功率等指标
- **优雅退出** —— 信号处理 (Ctrl+C / SIGTERM) 安全关闭
- **Windows 崩溃转储** —— 未处理异常时自动生成 `.dmp` 文件供事后分析
- **C++11 兼容** —— 仅依赖 C++11 和 pthread，兼容 GCC 4.8+ / MSVC 2015+
